/*
 ...
 Title: Full Model LeNet-5 Parallelized with MPI
 Description: Parallelized data preprocessing to improve the CNN's training efficiency by distributing the data loading and normalization across multiple MPI processes.
*/
...
void load_mnist_images(const char *file_path, float **images, int *num_images, int rank, int size) {
    ...
    int total_images = read_int(file);
    int rows = read_int(file);
    int cols = read_int(file);

    // Compute number of images per process
    int images_per_process = total_images / size;
    int remainder = total_images % size;
    int start_image = rank * images_per_process + (rank < remainder ? rank : remainder);
    int num_images_to_read = images_per_process + (rank < remainder ? 1 : 0);

    *num_images = num_images_to_read;

    // Seek to the start position
    int header_size = 16; // 4 * 4 bytes
    int image_size = rows * cols;

    fseek(file, header_size + start_image * image_size, SEEK_SET);

    // Allocate memory
    *images = (float *)malloc(num_images_to_read * IMAGE_DIMENSION * IMAGE_DIMENSION * sizeof(float));
...
void load_mnist_labels(const char *file_path, int **labels, int *num_labels, int rank, int size) {
    FILE *file = fopen(file_path, "rb");
    if (!file) {
    ...
    int total_labels = read_int(file);

    // Compute number of labels per process
    int labels_per_process = total_labels / size;
    int remainder = total_labels % size;
    int start_label = rank * labels_per_process + (rank < remainder ? rank : remainder);
    int num_labels_to_read = labels_per_process + (rank < remainder ? 1 : 0);

    *num_labels = num_labels_to_read;

    // Seek to the start position
    int header_size = 8; // 4 * 2 bytes

    fseek(file, header_size + start_label, SEEK_SET);

    // Allocate memory
    *labels = (int *)malloc(num_labels_to_read * sizeof(int));
    if (!*labels) {
...
void train_neural_network(float *images, int *labels, int num_images, int num_epochs, float learning_rate,
    ...
    for (int epoch = 0; epoch < num_epochs; ++epoch) {
        ...
        // Aggregate total_loss and correct across processes
        float global_total_loss;
        int global_correct;
        MPI_Allreduce(&total_loss, &global_total_loss, 1, MPI_FLOAT, MPI_SUM, MPI_COMM_WORLD);
        MPI_Allreduce(&correct, &global_correct, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);

        // Average weights across processes
        MPI_Allreduce(MPI_IN_PLACE, conv1_weights, LAYER1_CHANNELS * KERNEL_SIZE * KERNEL_SIZE, MPI_FLOAT, MPI_SUM, MPI_COMM_WORLD);
        MPI_Allreduce(MPI_IN_PLACE, conv1_biases, LAYER1_CHANNELS, MPI_FLOAT, MPI_SUM, MPI_COMM_WORLD);

        MPI_Allreduce(MPI_IN_PLACE, conv2_weights, LAYER2_CHANNELS * KERNEL_SIZE * KERNEL_SIZE, MPI_FLOAT, MPI_SUM, MPI_COMM_WORLD);
        MPI_Allreduce(MPI_IN_PLACE, conv2_biases, LAYER2_CHANNELS, MPI_FLOAT, MPI_SUM, MPI_COMM_WORLD);

        MPI_Allreduce(MPI_IN_PLACE, fc1_weights, FULLY_CONNECTED1_NEURONS * flattened_length, MPI_FLOAT, MPI_SUM, MPI_COMM_WORLD);
        MPI_Allreduce(MPI_IN_PLACE, fc1_biases, FULLY_CONNECTED1_NEURONS, MPI_FLOAT, MPI_SUM, MPI_COMM_WORLD);

        MPI_Allreduce(MPI_IN_PLACE, fc2_weights, OUTPUT_NEURONS * FULLY_CONNECTED1_NEURONS, MPI_FLOAT, MPI_SUM, MPI_COMM_WORLD);
        MPI_Allreduce(MPI_IN_PLACE, fc2_biases, OUTPUT_NEURONS, MPI_FLOAT, MPI_SUM, MPI_COMM_WORLD);

        // Divide weights by size to get the average
        for (int i = 0; i < LAYER1_CHANNELS * KERNEL_SIZE * KERNEL_SIZE; ++i)
            conv1_weights[i] /= size;
        for (int i = 0; i < LAYER1_CHANNELS; ++i)
            conv1_biases[i] /= size;

        for (int i = 0; i < LAYER2_CHANNELS * KERNEL_SIZE * KERNEL_SIZE; ++i)
            conv2_weights[i] /= size;
        for (int i = 0; i < LAYER2_CHANNELS; ++i)
            conv2_biases[i] /= size;
        ...
        float avg_loss = global_total_loss / (num_images * size);
        float accuracy = (float)global_correct / (num_images * size) * 100;
        if (rank == 0) {
...
void test_neural_network(float *images, int *labels, int num_images, float *conv1_weights, float *conv1_biases,
                         ...
                         float *fc2_weights, float *fc2_biases, int rank, int size) {
    ...
    // Aggregate correct predictions across all processes
    int global_correct_predictions;
    int total_images;
    MPI_Allreduce(&correct_predictions, &global_correct_predictions, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
    MPI_Allreduce(&num_images, &total_images, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);

    if (rank == 0) {
        printf("Test Accuracy: %.2f%%\n", (float)global_correct_predictions / total_images * 100);
    }
...
int main(int argc, char *argv[]) {
    ...
    if (strcmp(mode, "train") == 0) {
        ...
        load_mnist_images("train-images.idx3-ubyte", &train_images, &num_train_images, rank, size);
        load_mnist_labels("train-labels.idx1-ubyte", &train_labels, &num_train_labels, rank, size);
        ...
        train_neural_network(train_images, train_labels, num_train_images, num_epochs, learning_rate,
                             ...
                             fc1_weights, fc1_biases, fc2_weights, fc2_biases, rank, size);
        ...
        load_mnist_images("t10k-images.idx3-ubyte", &test_images, &num_test_images, rank, size);
        load_mnist_labels("t10k-labels.idx1-ubyte", &test_labels, &num_test_labels, rank, size);
        ...
        test_neural_network(test_images, test_labels, num_test_images, conv1_weights, conv1_biases,
                            conv2_weights, conv2_biases, fc1_weights, fc1_biases, fc2_weights, fc2_biases, rank, size);
