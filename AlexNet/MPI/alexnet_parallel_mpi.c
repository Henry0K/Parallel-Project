...
// Convolution Operation with MPI parallelization
void convolution_operation(float *input, float *filters, float *output,
    ...
    int channels_per_proc = output_channels / size;
    int remainder = output_channels % size;
    int start_channel = rank * channels_per_proc + (rank < remainder ? rank : remainder);
    int end_channel = start_channel + channels_per_proc + (rank < remainder ? 1 : 0);
    int local_output_channels = end_channel - start_channel;

    size_t local_output_size = local_output_channels * output_height * output_width;
    float *local_output = (float *)calloc(local_output_size, sizeof(float));

    for (int o = start_channel, local_o = 0; o < end_channel; ++o, ++local_o) {
        for (int h = 0; h < output_height; ++h) {
            for (int w = 0; w < output_width; ++w) {
                ...
                local_output[local_o * output_height * output_width + h * output_width + w] = acc;
            }
        }
    }

    // Gather local outputs to the root process
    int *recvcounts = NULL;
    int *displs = NULL;
    if (rank == 0) {
    ...
    int local_count = local_output_channels * output_height * output_width;
    ...
    MPI_Gatherv(local_output, local_count, MPI_FLOAT,
                output, recvcounts, displs, MPI_FLOAT, 0, MPI_COMM_WORLD);
    ...
    MPI_Bcast(output, output_channels * output_height * output_width, MPI_FLOAT, 0, MPI_COMM_WORLD);
}
...
// Fully Connected Layer with MPI parallelization
void dense_layer(float *input, float *weights, float *biases, float *output, int input_length, int output_length,
                 int rank, int size) {
    int neurons_per_proc = output_length / size;
    int remainder = output_length % size;
    ...
    int local_output_length = end_neuron - start_neuron;

    float *local_output = (float *)calloc(local_output_length, sizeof(float));

    for (int o = start_neuron, local_o = 0; o < end_neuron; ++o, ++local_o) {
        float sum = biases[o];
        for (int i = 0; i < input_length; ++i) {
            sum += input[i] * weights[o * input_length + i];
        }
        local_output[local_o] = sum;
    }
    ...
    int local_count = local_output_length;
    MPI_Gather(&local_count, 1, MPI_INT, recvcounts, 1, MPI_INT, 0, MPI_COMM_WORLD);
    ...
    // Broadcast the complete output to all processes
    MPI_Bcast(output, output_length, MPI_FLOAT, 0, MPI_COMM_WORLD);
}
...
void neural_network(float *input,
    ...
    double conv3_start_time = MPI_Wtime();
    ...
    convolution_operation(pool2_output, conv3_weights, conv3_output,
                          ...
                          rank, size);
    double conv3_end_time = MPI_Wtime();
    double conv3_time = conv3_end_time - conv3_start_time;
    ...
    double conv4_start_time = MPI_Wtime();
    ...
    convolution_operation(conv3_output, conv4_weights, conv4_output,
                          ...
                          rank, size);
    double conv4_end_time = MPI_Wtime();
    double conv4_time = conv4_end_time - conv4_start_time;
    ...
    double conv5_start_time = MPI_Wtime();
    ...
    convolution_operation(conv4_output, conv5_weights, conv5_output,
                          ...
                          rank, size);
    double conv5_end_time = MPI_Wtime();
    double conv5_time = conv5_end_time - conv5_start_time;
    ...
    double pool3_start_time = MPI_Wtime();
    ...
    free(conv5_output);
    double pool3_end_time = MPI_Wtime();
    double pool3_time = pool3_end_time - pool3_start_time;

    // Flatten the output for Fully Connected Layers
    int flattened_length = CONV5_FILTERS * pool3_output_height * pool3_output_width;

    // Fully Connected Layer 6
    double fc6_start_time = MPI_Wtime();
    float *fc6_output = (float *)malloc(FC6_NEURONS * sizeof(float));
    dense_layer(pool3_output, fc6_weights, fc6_biases, fc6_output, flattened_length, FC6_NEURONS, rank, size);
    double fc6_end_time = MPI_Wtime();
    double fc6_time = fc6_end_time - fc6_start_time;
    activate_relu(fc6_output, FC6_NEURONS);

    free(pool3_output);

    // Fully Connected Layer 7
    double fc7_start_time = MPI_Wtime();
    float *fc7_output = (float *)malloc(FC7_NEURONS * sizeof(float));
    dense_layer(fc6_output, fc7_weights, fc7_biases, fc7_output, FC6_NEURONS, FC7_NEURONS, rank, size);
    double fc7_end_time = MPI_Wtime();
    double fc7_time = fc7_end_time - fc7_start_time;
    activate_relu(fc7_output, FC7_NEURONS);

    free(fc6_output);

    // Fully Connected Layer 8 (Output Layer)
    double fc8_start_time = MPI_Wtime();
    float *fc8_output = (float *)malloc(OUTPUT_NEURONS * sizeof(float));
    dense_layer(fc7_output, fc8_weights, fc8_biases, fc8_output, FC7_NEURONS, OUTPUT_NEURONS, rank, size);
    double fc8_end_time = MPI_Wtime();
    double fc8_time = fc8_end_time - fc8_start_time;
    ...
    // Only rank 0 prints the timing information
    if (rank == 0) {
        ...
        save_timing_info("../Data/mpi_execution_time.txt", conv1_time, pool1_time, conv2_time, pool2_time, conv3_time, conv4_time, conv5_time, pool3_time, fc6_time, fc7_time, fc8_time, total_time);
    }
}
...
int main(int argc, char *argv[]) {
    ...
    load_data("../Data/input_image.bin", image, image_size);
    load_data("../Data/conv1_weights.bin", conv1_weights, conv1_weights_size);
    load_data("../Data/conv1_biases.bin", conv1_biases, CONV1_FILTERS);
    load_data("../Data/conv2_weights.bin", conv2_weights, conv2_weights_size);
    load_data("../Data/conv2_biases.bin", conv2_biases, CONV2_FILTERS);
    load_data("../Data/conv3_weights.bin", conv3_weights, conv3_weights_size);
    load_data("../Data/conv3_biases.bin", conv3_biases, CONV3_FILTERS);
    load_data("../Data/conv4_weights.bin", conv4_weights, conv4_weights_size);
    load_data("../Data/conv4_biases.bin", conv4_biases, CONV4_FILTERS);
    load_data("../Data/conv5_weights.bin", conv5_weights, conv5_weights_size);
    load_data("../Data/conv5_biases.bin", conv5_biases, CONV5_FILTERS);
    load_data("../Data/fc6_weights.bin", fc6_weights, fc6_weights_size);
    load_data("../Data/fc6_biases.bin", fc6_biases, FC6_NEURONS);
    load_data("../Data/fc7_weights.bin", fc7_weights, fc7_weights_size);
    load_data("../Data/fc7_biases.bin", fc7_biases, FC7_NEURONS);
    load_data("../Data/fc8_weights.bin", fc8_weights, fc8_weights_size);
    load_data("../Data/fc8_biases.bin", fc8_biases, OUTPUT_NEURONS);
